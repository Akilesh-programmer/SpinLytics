...
export default function DispatchScreen() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setToast({ visible: true, message: 'Dispatch entry saved!', type: 'success' });
      setForm((p) => ({
    ...
    <View style={styles.wrapper}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <ScreenHeader icon="send-outline" title="Dispatch Entry" description="Record dispatches to parties. Reduces stock, converts bags to kg." color="#f59e0b" />
          ...
          <SubmitButton title="Save Dispatch Entry" onPress={submit} loading={loading} />
        </ScrollView>
