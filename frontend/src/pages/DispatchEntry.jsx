...
export default function DispatchEntry() {
  const [form, setForm] = useState({
    date: today(), materialType: 'YARN', lotNo: '', partyName: '', bags: '', pricePerBag: '', remarks: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast('Dispatch entry saved');
  ...
  return (
    <div className="entry-form-page" id="dispatch-entry">
      <div className="page-header">
        <h1 className="page-title"><Truck size={24} /> Dispatch Entry</h1>
      </div>
      ...
      <form className="entry-form" onSubmit={handleSubmit}>
        <div className="entry-form-grid">
          ...
          <div className="form-group">
            ...
            <input value={form.partyName} onChange={(e) => update('partyName', e.target.value)} placeholder="XYZ Mills" required />
          </div>
          <div className="form-group">
            <label>Bags</label>
            <input type="number" step="any" value={form.bags} onChange={(e) => update('bags', e.target.value)} placeholder="5" required />
            <span className="auto-calc-label">= {kgs} kg</span>
          </div>
          <div className="form-group">
            <label>Price per Bag (₹)</label>
            <input type="number" step="any" value={form.pricePerBag} onChange={(e) => update('pricePerBag', e.target.value)} placeholder="6000" />
            {totalPrice !== '—' && <span className="auto-calc-label">Total: ₹{totalPrice}</span>}
          </div>
          <div className="form-group full-width">
            <label>Remarks</label>
            <textarea value={form.remarks} onChange={(e) => update('remarks', e.target.value)} placeholder="Invoice #1234..." rows={2} />
          </div>
        </div>

        <div className="entry-form-actions">
          <button className="btn btn-primary" type="submit" disabled={saving}>
            {saving ? 'Saving...' : <><Check size={16} /> Save Dispatch Entry</>}
          </button>
        </div>
      </form>
